...
for basis in ['tz']:
	y = pd.read_csv('./'+basis+'.bk1.0.csv',skip_blank_lines=True,skipinitialspace=True)
...
for basis in ['tz']:
        z = pd.read_csv('./'+basis+'.bk1.0.csv',skip_blank_lines=True,skipinitialspace=True)
...
ff['bk1.0'] = stu['tz_ccsd']
#ff['bk1.0 SCF'] = hf['tz_scf'] 
ff['bk1.0 gaps'] = ff['bk1.0'].values - ff['bk1.0'].values[0]
#ff['ECP corr. gaps'] = ff['bk1.0 gaps'] - (ff['bk1.0 SCF'].values - ff['bk1.0 SCF'].values[0])
#ff['opt'] = (ff['ae gaps'] - ff['ECP corr. gaps'])*toev
ff['Error'] = ff['ae gaps'] - ff['bk1.0 gaps']
#ff['ECP_corr'] = ff['bk1.0'] - ff['bk1.0 SCF']
