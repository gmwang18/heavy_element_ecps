...
for j in range(unique_orbs):
    ...
    orbital_end = float(data_set[j][0][-1])
    cut_marker = int(0) # will be a negative number indexing from the end
    k = int(0)
    ...
    t = InterpolatedUnivariateSpline(data_set[j][0], data_set[j][1], k=3)
    temp_y = t(data_set[j][0])
    psi_d   = t(data_set[j][0],1)
    psi_dd  = t(data_set[j][0],2)
    psi_ddd = t(data_set[j][0],3)
    pyl.close()
    pyl.subplot(411)
    pyl.title('original (red) vs interpolation spline (blue)')
    pyl.semilogy(data_set[j][0], data_set[j][1],'-r')
    pyl.semilogy(data_set[j][0], temp_y,'-b')
    pyl.subplot(412)
    pyl.xlabel('r')
    pyl.ylabel('d_r Psi(r)')
    pyl.semilogy(data_set[j][0], abs(psi_d), '-b')
    pyl.subplot(413)
    pyl.xlabel('r')
    pyl.ylabel('dd_r Psi(r)')
    pyl.semilogy(data_set[j][0], abs(psi_dd), '-b')
    pyl.subplot(414)
    pyl.xlabel('r')
    pyl.ylabel('ddd_r Psi(r)')
    pyl.semilogy(data_set[j][0], abs(psi_ddd), '-b')
    pyl.show()
    # collect the tail data points in temporaray storage
    temp_x = []
    temp_y = []
...
#    pyl.close()
#    pyl.title('original (red) vs exp tail (blue)')
#    pyl.semilogy(data_set[j][0], data_set[j][1],'-r')
#    pyl.semilogy(data_set[j][0], fit_curve,'-b')
#    pyl.show()    
    u = InterpolatedUnivariateSpline(data_set[j][0], fit_curve, k=3)
#    temp_y = u(data_set[j][0])
#    psi_d   = u(data_set[j][0],1)
#    psi_dd  = u(data_set[j][0],2)
#    psi_ddd = u(data_set[j][0],3)
#    pyl.close()
#    pyl.subplot(411)
#    pyl.title('original (red) vs interpolation of tail (blue)')
#    pyl.semilogy(data_set[j][0], data_set[j][1],'-r')
#    pyl.semilogy(data_set[j][0], temp_y,'-b')
#    pyl.subplot(412)
#    pyl.xlabel('r')
#    pyl.ylabel('d_r Psi(r)')
#    pyl.semilogy(data_set[j][0], abs(psi_d), '-r')
#    pyl.subplot(413)
#    pyl.xlabel('r')
#    pyl.ylabel('dd_r Psi(r)')
#    pyl.semilogy(data_set[j][0], abs(psi_dd), '-b')
...
#    pyl.xlabel('r')
#    pyl.ylabel('ddd_r Psi(r)')
#    pyl.semilogy(data_set[j][0], abs(psi_ddd), '-g')
#    pyl.show()
    # buid splice orbital data from original and exponential-fit tail
    splice_x = []
    splice_y = []
    leftedge = len(data_set[j][0]) + ( cut_marker - 10 )
    print leftedge, data_set[j][0][leftedge]
    rightedge = len(data_set[j][0]) + ( cut_marker + 10 )
    print rightedge, data_set[j][0][rightedge] 
    for i in range(len(data_set[j][0])):
        if i < leftedge:
        ...
        if i >= leftedge:
            if i < rightedge:
                denom = data_set[j][0][rightedge] - data_set[j][0][leftedge]
                A = (data_set[j][0][rightedge]-data_set[j][0][i])/denom
                B = (data_set[j][0][i]-data_set[j][0][leftedge])/denom
                C = (pow(A,3)-A)*(pow(denom,2))/6
                D = (pow(B,3)-B)*(pow(denom,2))/6
                temp = A*data_set[j][1][leftedge]
                temp += B*tail(data_set[j][0][rightedge],popt[0],popt[1]) 
                temp += C*t(data_set[j][0][leftedge],2)[0] 
                ydprime_right = u(data_set[j][0][rightedge],2)[0]
#                ydprime_right = tail(data_set[j][0][rightedge],popt[0],popt[1])*pow(popt[1],2)
                temp += D*ydprime_right
                splice_x.append(data_set[j][0][i])
                splice_y.append(temp)
            if i >= rightedge:
                splice_x.append(data_set[j][0][i])
                splice_y.append(tail(data_set[j][0][i],popt[0],popt[1]))
#    pyl.close()
#    pyl.title('original (red) vs spliced curve (blue)')
#    pyl.semilogy(data_set[j][0], data_set[j][1],'-r')
#    pyl.semilogy(splice_x, splice_y,'-b')
#    pyl.show()
    ...
    #        else:
    #            z += 1
    s = UnivariateSpline(splice_x, splice_y, k=3, s=0.0)
    ...
    pyl.xlabel('r')
    pyl.ylabel('dd_r Psi(r)')
    pyl.semilogy(output_set[j][0], abs(psi_dd), '-r')
    pyl.subplot(515)
    pyl.xlabel('r')
    pyl.ylabel('ddd_r Psi(r)')
    pyl.semilogy(output_set[j][0], abs(psi_ddd), '-r')
    pyl.show()
    # end of graphing
