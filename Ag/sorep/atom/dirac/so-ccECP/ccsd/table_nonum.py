...
for basis in ['dz','tz','qz']:
	x = pd.read_csv('./'+basis+'/'+basis+'.table.txt',skip_blank_lines=True,skipinitialspace=True,delim_whitespace=True)
	df[basis+'_scf'] = x['SCF']
...
total = pd.DataFrame()
total['Total_Gap'] = (hf['Extrap.Gaps'] + corr['Extrap.Gaps']) * toev
