...
for j in range(unique_orbs):
    ...
    # remove 0.0's at the end in phi(r) up to 1E-6
    while read_y[-1] < 1E-6:
...
for j in range(unique_orbs):
    ...
    t = InterpolatedUnivariateSpline(data_set[j][0], data_set[j][1], k=5)
#    print data_set[j][0][0], data_set[j][1][0]
#    print t(0.02), t(0.02,1), t(0.02,2), t(0.02,3)
#    print t(cut-0.0001), t(cut-0.0001,1), t(cut-0.0001,2), t(cut-0.0001,3)
#    print t(cut), t(cut,1), t(cut,2), t(cut,3)
#    print t(cut+0.0001), t(cut+0.0001,1), t(cut+0.0001,2), t(cut+0.0001,3)
    ...
    # smoothing the two together over 10 grid points of the input x
    splice_x = []
    splice_y = []
    leftedge = len(data_set[j][0]) + ( cut_marker - 5 )
    rightedge = len(data_set[j][0]) + ( cut_marker + 5 )
    ...
    for i in range(len(data_set[j][0])):
        ...
        if i > leftedge:
            if i < rightedge:
                A = (data_set[j][0][rightedge]-data_set[j][0][i])/width
                B = (data_set[j][0][i]-data_set[j][0][leftedge])/width
                C = (pow(A,3)-A)*(pow(width,2))/6
                D = (pow(B,3)-B)*(pow(width,2))/6
                temp = A*data_set[j][1][leftedge] 
                temp += B*tail(data_set[j][0][rightedge],popt[0],popt[1]) 
                temp += C*t(data_set[j][0][leftedge],2) 
                ydprime_right = popt[1]*popt[1]*tail(data_set[j][0][rightedge],popt[0],popt[1])
    ...
    while k < splice_x[-1]:
        ...
        k += step_size    
    s = InterpolatedUnivariateSpline(splice_x, splice_y, k=5)
    output_x = splice_x
    output_y = splice_y
    psi_d   = t(output_x,1)
    psi_dd  = t(output_x,2)
    psi_ddd = t(output_x,3)
