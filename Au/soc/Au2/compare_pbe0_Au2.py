...
df_cc = pd.read_csv("ccsd-t-arep-mdfstu/tz.csv", index_col=False, engine='python', sep='\s*,\s*')
...
atom_pbe0dmc = ufloat(-135.7376327, 0.0008142720626)
df_binding['PBE0DMC'] = df_pbe0dmc['energy'].values - 2.0*atom_pbe0dmc

df_cc_ccECP = pd.read_csv("ccsd-t-arep-ccECP/tz.csv", index_col=False, engine='python', sep='\s*,\s*')
...
columns = df_binding.columns
blatex=pd.DataFrame(columns=columns,  index=["$D_e$(eV)", "$r_e$(\AA)", "$\omega_e$(cm$^{-1}$)"])
#blatex=pd.DataFrame(index=["$D_e$(eV)", "$r_e$(\AA)", "$\omega_e$(cm$^{-1}$)"])
#print(blatex)
del blatex['Z']
...
ax.axvline(2.4719, color='r', linestyle='-',  linewidth=1.5, label='Expt. $r_{eq}$')
...
for column in df_binding:
        if column == "Z":
                ...
                print(column)
                initial=[0.080, 1.70, 2.5]  # initial guess
                print(initial)
...
plt.savefig('Au2_PBE0_compare_binding.pdf')
#plt.savefig('Au2_PBE0_compare_binding.png', dpi=800)
plt.show()
