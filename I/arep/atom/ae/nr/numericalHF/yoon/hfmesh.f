...
c  this was added for working with lmitas's code...
	...
	if (ichar.eq.acg) then
	  ...
	  njrc(4)=1
	  do 52 i=1,nr
	  call pseudg(r(i),vs,vp,vloc,vloc1)
	  vi(i,1)=vs+vloc
	  vi(i,3)=vp+vloc
	  vi(i,5)=vloc
	  vi(i,7)=vloc
	...
	write (17,*) r(i),vp
	write (18,*) r(i),vloc1
	write (19,*) r(i),vs+vloc
	write (20,*) r(i),vp+vloc
			endif
 52	  continue
		rcutm=3.
		nmesh=3001
		...
		call pseudg(rmesh,vs,vp,vloc,vloc1)
	write (21,*) rmesh,2.*(vs+vloc),2.*(vp+vloc),2.*vloc
		enddo
	close(16)
        close(17)
        close(18)
...
c	for meshes
c
        if (ichar.eq.acm) then
          njrc(1)=1
          njrc(2)=1
          njrc(3)=1
          njrc(4)=1
          do i=1,nr
          call pseudm(r(i),vs,vp,vloc)
          vi(i,1)=vs
          vi(i,3)=vp
          vi(i,5)=vloc
          vi(i,7)=vloc
        if (r(i).lt.2.) then
        write (16,*) r(i),vs-vloc
        write (17,*) r(i),vp-vloc
        write (18,*) r(i),vloc
        write (19,*) r(i),vs
        write (20,*) r(i),vp
                        endif
