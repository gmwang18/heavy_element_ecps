...
def init():
    ...
    legend = {'frameon':True, 'fontsize':13, 'handlelength':2.30, 'labelspacing':0.30, 'handletextpad':0.5, 'loc':'best', 'facecolor':'white', 'framealpha':1.00, 'edgecolor':'#f2f2f2'}
...
df_cc = pd.read_csv("ccsd-t_arep-mdfstu/5z.csv", index_col=False, engine='python', sep='\s*,\s*')

print(df_cc)

bind_cc = df_cc['CCSD'] - 2* df_cc['A_CCSD'].values[0]
...
atom_pbe0dmc = ufloat(-5.383771322, 0.0001784346419)
df_binding['PBE0DMC'] = df_pbe0dmc['energy'].values - 2.0*atom_pbe0dmc

print(df_pbe0dmc)

df_cc_ccECP = pd.read_csv("ccsd-t_arep-ccECP/5z.csv", index_col=False, engine='python', sep='\s*,\s*')
bind_cc_ccECP = df_cc_ccECP['CCSD'] - 2* df_cc_ccECP['A_CCSD'].values[0]
...
atom_ccECP_pbe0dmc = ufloat(-5.438788603, 0.0002588484633)
...
ax.axvline(2.6596, color='r', linestyle='-',  linewidth=1.5, label='Expt. $r_{eq}$')
...
ax.axvline(ccecp_dmc_bind_geo, color='#339933', linestyle='--', linewidth=1.5, dashes=(8,2), label='ccECP $r_{eq}$')
...
ax.set_ylim([-3.0, -0.7])
plt.savefig('Bi2_PBE0_compare_binding.pdf')
#plt.savefig('Bi2_PBE0_compare_binding.png', dpi=800)
plt.show()
