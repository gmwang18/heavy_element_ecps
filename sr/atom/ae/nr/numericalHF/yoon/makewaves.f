...
c
	...
	write (*,*) ' l = ',lstat
		...
		xxi=-1.
		i=0
c       i is used as the term counter
c       later at the end of the loop, imax is set to i-1 or i-12
		dismin=0.001/zfull
			do 100 ii=1,nval
c			read (1,*) idummy,x(i),y(i),up,upp
			...
			y(i)=yyn
			xxi=xxn
c       In the following line, x(i) used to be raised to the (lstat+1)
c       power, but I've removed it to experiment
			y(i)=y(i)/(x(i))
 ...
 100		continue
	write (*,*) ' r(1) rho(1) = ', x(1),y(1), 'imax ', imax
        write (*,*) ' actual rmax= ',rmax
     &    , '  R(rmax)= ', y(imax)*(x(imax)**lstat)
c
	ne=3
	der0=(y(ne)-y(1))/(x(ne)-x(1))
	y0=y(1)+(0.-x(1))*der0
	write (*,*) y(1),y(ne),x(1),x(ne)
	write (*,*) ' orig der0 ',der0
	write (*,*) x(1),x(ne),y(1),y(ne)
	 derr0=float(lstat+1)*der0/y(1)
	write (*,*) ' der0 ', float(lstat+1)*der0/y(1)
	if (derr0.gt.150.)		then
	der0=0.
					else	
...
c
	imaxo=imax
	rextr=5./(1.+sqrt(zfull))
        rextr=min(rextr,x(imax))
c
	do j=imax+1,nval
	if(x(j)-x(imax).gt.rextr) go to 186
        enddo
186     continue
        rmaxs=x(j)
c
...
c	if (x(imax)-x(imaxo).lt.3.)			then
c		do 193 j=1,nval
c		if (x(imax)-x(j).le.3.)	go to 194
c193		continue
c194		imaxo=j	
c							endif
c
...
c
	...
	write (*,*) ' x(imax)',x(imax)
        write (*,*) 'interval cut',x(imax)-x(imaxo),imaxo,imax
...
c
	...
	parameter (nmax=4000,nn=4000)
        common /spl/ splcc(nn,4),x(nn),y(nn) 
	real xr,yr,yp1r,ypnr,splcr
	dimension xr(nn),yr(nn),splcr(nn,4)
c       splcr(4000,4) is the array holding spline coeffs
	dimension y2(nmax),u(nmax)
c       
	yp1=dble(yp1r)
c       yp1 now holds the der0 value
	ypn=dble(ypnr)
c       ypn now holds the dern value
		do 10 i=1,n
c       n holds imax value, determined by logic in main
		x(i)=dble(xr(i))
		y(i)=dble(yr(i))
c       x and y hold the x,y values of phi
 10		continue
c
