...
for basis in ['tz']:
	y = pd.read_csv('./basis/'+basis+'.table1.csv',skip_blank_lines=True,skipinitialspace=True)
        hf[basis+'_scf'] = y['SCF']

scf = hf.filter(regex='scf')
...
ff['CCSD'] = ccsd['tz_ccsd']
ff['SCF'] = scf['tz_scf']
ff['ECP Corr. Gaps'] = (ff['CCSD'].values - ff['CCSD'].values[0]) - (ff['SCF'].values - ff['SCF'].values[0])
