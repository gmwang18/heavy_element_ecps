...
for basis in ['tz']:
	y = pd.read_csv('./'+basis+'.mdfstu.csv',skip_blank_lines=True,skipinitialspace=True)
...
for basis in ['tz']:
        z = pd.read_csv('./'+basis+'.mdfstu.csv',skip_blank_lines=True,skipinitialspace=True)
...
ff['MDFSTU SCF'] = hf['tz_scf'] 
ff['MDFSTU gaps'] = ff['MDFSTU'].values - ff['MDFSTU'].values[0]
ff['ECP corr. gaps'] = ff['MDFSTU gaps'] - (ff['MDFSTU SCF'].values - ff['MDFSTU SCF'].values[0])
ff['opt'] = (ff['ae gaps'] - ff['ECP corr. gaps'])*toev
#ff['Error'] = ff['ae gaps'] - ff['MDFSTU gaps']
#ff['ECP_corr'] = ff['MDFSTU'] - ff['MDFSTU SCF']
...
print(ff['opt'].to_latex(index=False))
